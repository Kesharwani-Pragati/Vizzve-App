...
dynamic itemTotal;
double gstPrice = 0;
String? razorpayId,cashfreeId,cashfreeKey,
    paystackId,
    stripeId,
    stripeSecret,
    stripeMode = "test",
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget cartItem(int index, List<SectionModel> cartList) {
    ...
    var pack = cartList[index].productList![0].packing_charge.toString() == ""||cartList[index].productList![0].packing_charge==null
        ? "0.0"
        : (double.parse(cartList[index].productList![0].packing_charge.toString())*double.parse(cartList[index].qty.toString())).toStringAsFixed(2);
    cartList[index].perItemPrice = price.toString();
    cartList[index].perItemTotal =
        (price * double.parse(cartList[index].qty!)).toString();
    itemTotal = (double.parse(cartList[index].perItemTotal.toString()) +
        double.parse(pack)).toString();
          ...
          children: [
            ...
            cartList[index].productList![0].packing_charge.toString() == ""
              ...
              children: [
                ...
                cartList[index].productList![0].packing_charge.toString() == ""||cartList[index].productList![0].packing_charge==null
                    ?Container():Text(
                  CUR_CURRENCY! +
                      " " +
                      cartList[index]
                ...
                ?Text(
                  ...
                  CUR_CURRENCY! +
                      ...
                      (double.parse(cartList[index]
                          .productList![0]
                          .packing_charge
                          .toString())*double.parse(cartList[index]
                          .qty
      ...
      try {
        ...
        if (!error) {
            ...
            if(addressList.length>0){
              if (itemLong != 0.0 && itemLat != 0.0) {
                final double km = distance.as(
                    ...
                    new LatLng(double.parse(addressList[selectedAddress!].latitude.toString()), double.parse(addressList[selectedAddress!].longitude.toString())));
              ...
              if (itemLong != 0.0 && itemLat != 0.0) {
                final double km = distance.as(
                    LengthUnit.Kilometer,
                    new LatLng(itemLat, itemLong),
                    new LatLng(double.parse(loc.lat.toString()), double.parse(loc.lng.toString())));
                delCharge = await deliveryChargeApi(km);
              }
            }
          ...
          for (var i = 0; i < cartList.length; i++) {
            if (cartList[i].productList![0].packing_charge.toString() != ""&&cartList[i].productList![0].packing_charge != null) {
                                        ...
                                        return Container(
                                            ...
                                            decoration: BoxDecoration(
                                                color: Theme.of(context)
                                                    ...
                                                    .white,
                                                border: Border.all(
                                                    color: colorVariation((_resizableController!.value *10).round())!, width:3),
                                                borderRadius:
  ...
  doPayment() {
    if (payMethod == getTranslated(context, 'PAYPAL_LBL')) {
      ...
      CashFreeHelper cashFreeHelper = new CashFreeHelper(totalPrice.toString(), context, (result){
        print(result['txMsg']);
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            isPromoValid = true;
            Fluttertoast.showToast(msg: getTranslated(context, 'PROMO_SUCCESS')!,
              backgroundColor: colors.primary
            );
