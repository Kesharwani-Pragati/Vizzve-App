...
class StateProduct extends State<ProductList> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: widget.fromSeller! ? null : getAppBar(widget.name!, context,type: "Product"),
        key: _scaffoldKey,
  ...
  Widget listItem(int index) {
    if (index < productList.length) {
      ...
      if(model.breakfast_start_product_time!=null&&model.breakfast_end_product_time!=null){
        List hM = model.breakfast_start_product_time.toString().split(":").toList();
        List hM1 = model.breakfast_end_product_time.toString().split(":").toList();
       // print("${time.hour}:${time.minute}");
        DateTime t = DateTime(time.year ,time.month,time.day,int.parse(hM[0]),int.parse(hM[1]));
        DateTime m = DateTime(time.year ,time.month,time.day,int.parse(hM1[0]),int.parse(hM1[1]));
      ...
      if(model.lunch_start_product_time!=null&&model.lunch_end_product_time!=null){
        List hM = model.lunch_start_product_time.toString().split(":").toList();
        List hM1 = model.lunch_end_product_time.toString().split(":").toList();
        DateTime t = DateTime(time.year ,time.month,time.day,int.parse(hM[0]),int.parse(hM[1]));
        DateTime m = DateTime(time.year ,time.month,time.day,int.parse(hM1[0]),int.parse(hM1[1]));
        if(time.isAfter(t)&&time.isBefore(m)){
          timeData = "Yes";
      ...
      if(timeData!="Yes"&&timeData==""){
        if(model.dinner_start_product_time!=null&&model.dinner_end_product_time!=null){
          List hM = model.dinner_start_product_time.toString().split(":").toList();
          List hM1 = model.dinner_end_product_time.toString().split(":").toList();
          //  print("${time.hour}:${time.minute}");
          DateTime t = DateTime(time.year ,time.month,time.day,int.parse(hM[0]),int.parse(hM[1]));
          DateTime m = DateTime(time.year ,time.month,time.day,int.parse(hM1[0]),int.parse(hM1[1]));
          if(time.isAfter(t)&&time.isBefore(m)){
      ...
      if(model.breakfast_start_product_time==null&&model.breakfast_end_product_time==null&&model.lunch_start_product_time==null&&model.lunch_end_product_time==null&&model.dinner_start_product_time==null&&model.dinner_end_product_time==null){
        timeData = "No";
      }else{
        if(model.dinner_end_product_time!=null&&model.dinner_start_product_time!=null){
          List hM = model.dinner_start_product_time.toString().split(":").toList();
          DateTime t = DateTime(time.year ,time.month,time.day,int.parse(hM[0]),int.parse(hM[1]));
          if(time.isAfter(t)){
              if(model.breakfast_start_product_time!=null){
                timeData = model.breakfast_start_product_time.toString()+",Tomorrow";
              }else if(model.lunch_start_product_time!=null){
                timeData = model.lunch_start_product_time.toString()+",Tomorrow";
              }else if(model.dinner_start_product_time!=null){
                timeData = model.dinner_start_product_time.toString()+",Tomorrow";
              }
          }
          ...
          List hM = model.breakfast_end_product_time.toString().split(":").toList();
          DateTime t = DateTime(time.year ,time.month,time.day,int.parse(hM[0]),int.parse(hM[1]));
          if(time.isAfter(t)){
            if(model.breakfast_start_product_time!=null){
              timeData = model.breakfast_start_product_time.toString()+",Tomorrow";
...
getTime(data){
    String time = data.toString().split(",").length>0?data.toString().split(",")[0]:"10:00";
    String ext = data.toString().split(",").length>1?data.toString().split(",")[1]:"";
    DateTime date =DateTime(DateTime.now().year,DateTime.now().month,DateTime.now().day,int.parse(time.toString().split(":")[0]),int.parse(time.toString().split(":")[1]));
  ...
  _showForm(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        checkOut.curCartCount!=""&&checkOut.curCartCount!=null&&int.parse(checkOut.curCartCount) > 0?
        Container(
